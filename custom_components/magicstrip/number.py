    ...
    def _constructor(device_state: DeviceState) -> list[Entity]:
        return [
            MagicStripEffectSpeed(
                device_state.coordinator, device_state.device, device_state.effect_speed_device_info, device_state.light_extra_state_attributes
            )
        ]
...
class MagicStripEffectSpeed(CoordinatorEntity[MagicStripState], NumberEntity):
    ...
    def value(self) -> float | None:
        ...
        if data := self.coordinator.data:
            ...
            _LOGGER.debug("Retrieving speed. Stored: {}, Rebased: {}", speed, rebased_speed)
